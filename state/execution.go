...
type BlockExecutor struct {
	// save state, validators, consensus params, ocabci responses here
	store Store
...
// mempool.
// It returns the result of calling ocabci.Commit (the AppHash) and the height to retain (if any).
...
func getBeginBlockValidatorInfo(block *types.Block, store Store,
	initialHeight int64, voterParams *types.VoterParams) ocabci.LastCommitInfo {
...
// It returns the application root hash (result of ocabci.Commit).
