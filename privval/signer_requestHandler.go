...
func DefaultValidationRequestHandler(
	...
	req oprivvalproto.Message,
	chainID string,
) (oprivvalproto.Message, error) {
	var (
		res oprivvalproto.Message
		err error
	)

	switch r := req.Sum.(type) {
	case *oprivvalproto.Message_PubKeyRequest:
		if r.PubKeyRequest.GetChainId() != chainID {
			res = mustWrapMsg(&oprivvalproto.PubKeyResponse{
		...
		if err != nil {
			...
			res = mustWrapMsg(&oprivvalproto.PubKeyResponse{PubKey: pk, Error: nil})
		}

	case *oprivvalproto.Message_SignVoteRequest:
	...
	case *oprivvalproto.Message_SignProposalRequest:
	...
	case *oprivvalproto.Message_PingRequest:
		err, res = nil, mustWrapMsg(&privvalproto.PingResponse{})

	case *oprivvalproto.Message_VrfProofRequest:
		...
		if err != nil {
			...
			res = mustWrapMsg(&oprivvalproto.VRFProofResponse{Proof: nil, Error: &err})
		} else {
			res = mustWrapMsg(&oprivvalproto.VRFProofResponse{Proof: proof[:], Error: nil})
		}
