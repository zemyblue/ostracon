...
func (app *Application) CheckTxSync(req tmabci.RequestCheckTx) types.ResponseCheckTx {
	return app.checkTx(req)
}

func (app *Application) CheckTxAsync(req tmabci.RequestCheckTx, callback types.CheckTxCallback) {
	callback(app.checkTx(req))
}

func (app *Application) checkTx(req tmabci.RequestCheckTx) types.ResponseCheckTx {
	if app.serial {
		if len(req.Tx) > 8 {
